...
const SlaveContainer = (props) => {
  ...
  // relevanceInstant 不管有没有mongo 都会重新查询
  const columns = [
    ...
    { title: '字段|类型', dataIndex: 'containerType', fieldType: 'varchar', dropType: 'const',
      viewDrop: '{ "field": "字段", "fieldNotSelect": "字段不查询", "relevance": "关联性字段", "relevanceNotView": "关联性不展现字段", "relevanceInstant": "关联性即时字段", "spare": "备用字段", "control": "控件", "cascader": "级联" }', defaultValue: 'field', sortNum: 38, width: 150 },
  ...
  const onClick = async (name, e) => {
    ...
    if (name === 'slaveAddButton') {
      ...
      const url: string = application.urlPrefix + '/container/getDBFields?tableName=' + containerName;
      ...
      if (navigator.clipboard) {
        ...
        if (commonUtils.isJson(clipboardText)) {
          const slaveData = [...slaveDataOld];
